...
- Shorthand: `log: <food>` → gohan:log-meal · `weight: <kg>` / `gym: <what, duration>` / `treatment: <note>` → gohan:log-body · `summary [range]` → gohan:summarize · `dashboard [range]` → gohan:dashboard.
EOF

...
// apps/scripts/src/import-indb.ts
...
// apps/scripts/src/import-indb.ts
var INDB_URL = "https://www.anuvaad.org.in/wp-content/uploads/2020/07/Anuvaad_INDB_2024.11.xlsx";
var OUT = "data/reference/indb.json";
var PARENTHETICAL = /\(([^)]*)\)/g;
var NON_ALPHANUMERIC = /[^a-z0-9]+/g;
var EDGE_DASHES = /^-+|-+$/g;
var MICRO_FIELDS = [
  "freesugar_g",
  "sfa_mg",
...
function slugify(name) {
  return name.toLowerCase().replace(PARENTHETICAL, "").replace(NON_ALPHANUMERIC, "-").replace(EDGE_DASHES, "");
}
function aliasesOf(name) {
  return [...name.matchAll(PARENTHETICAL)].map((m) => (m[1] ?? "").trim().toLowerCase()).filter((a) => a.length > 0);
}
function round(value, dp = 3) {
  const f = 10 ** dp;
  return Math.round(Number(value) * f) / f;
}
function micros(row, prefix) {
  const out = {};
...
async function loadWorkbook() {
  ...
  if (localPath) {
    return readSync(new Uint8Array(await readFile(localPath)), { type: "array" });
  }
  const response = await fetch(INDB_URL);
  if (!response.ok) {
    throw new Error(`INDB download failed: ${response.status} ${INDB_URL}`);
  }
  return readSync(new Uint8Array(await response.arrayBuffer()), { type: "array" });
}
var workbook = await loadWorkbook();
...
var entries = rows.map((row) => {
  const name = String(row.food_name);
  const baseId = slugify(name);
  const count = idCounts.get(baseId) ?? 0;
  idCounts.set(baseId, count + 1);
  const hasServing = row.unit_serving_energy_kcal != null && Number(row.energy_kcal) > 0;
  return {
    ...
    servingSizeG: hasServing ? round(Number(row.unit_serving_energy_kcal) / Number(row.energy_kcal) * 100, 1) : null,

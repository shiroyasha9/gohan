...
Write the meal into the day-file with a full resolution trail, grounded in the personal food cache. Authoritative shapes: `packages/core/src/schemas.ts`. Conventions (wake-day, confidence, commit protocol): `CLAUDE.md`.
...
7. **Write the trail.** Item carries: `input` (user's words verbatim), `food`, `foodId`, `qty {amount, unit, grams, basis}`, `nutrients`, `source` (copied from the cache entry), `confidence` per CLAUDE.md.
8. **Validate.** `bun run validate <day-file>` must pass.

...
Fast entries into the day-file's `body` and `activity` sections. Shapes: `packages/core/src/schemas.ts`.
...
3. `bun run validate <day-file>`, then the commit protocol (CLAUDE.md).

...
The chart is ground truth for adherence — transcribe it faithfully, never paraphrase options. Shapes: `packages/core/src/schemas.ts`.
...
4. `bun run validate`, then `bun run summary` to show the derived kcal/protein envelope with any unresolved options named.
5. Commit protocol.

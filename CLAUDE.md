...
- `apps/scripts` — CLI sources (validate, summary, recompute, import-indb). They resolve `data/` relative to cwd, i.e. the user's data repo.
- `plugin/` — the shipped surface: `skills/` (all 8), `scripts/` (prebuilt bun bundles — generated, never hand-edit), `schemas.ts` (copy), `hooks/` (guarded SessionStart conventions).
